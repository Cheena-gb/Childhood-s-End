...
<details><sumamry>LEDが正しくはんだづけできているか不安…</sumamry>コンスルーを使用し、基板とMCUを接続してみてください。問題なくはんだづけできれていれば、通電すると赤色に光るはずです。この状態でVialを起動し、Backlightingのタブから色やアニメーションを切り替えましょう。問題なく接続できているなら、なめらかに切り替わりますが、接続が不安定だと、その部分から先のLEDは不安定になります。</details>
...
### プレートの組み立て
スイッチプレートの端数か所にスイッチを挿入し、基板のソケットに挿入して仮固定します。  
この状態でスイッチプレートの表からネジを通し、基板の六角形の穴にスペーサーを通してねじ止めしていきます。  
すべてのスペーサーを固定したら、スイッチを挿入します。  
この時、ソケットを抑えて基板の脱落を防ぎます。  
すべてのスイッチを取り付けたら、バックプレートをねじ止めしてください。
