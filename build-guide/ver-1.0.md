...
### 内容物の確認
 ...
 - スイッチプレート  
   1枚入っています。星のマークのある面は裏側(組み立て後は内側)となります。
 - バックプレート  
   1枚入っています。文字の掛かれた面が表側(組み立て後は外側)となります。  
 - ゴム足  
   小4個大2個、合計6枚入っています。
 - ネジ  
   長短10本ずつ入っています。
...
LEDを実装している場合、MCUに通電すれば発光するはずです。

### トラブルシューティング(1)
<details><summary>LEDがすべて光らない</summary>D73、L1は問題なくはんだ付けできていますか？</details>  
...
### 導通確認
VialのMatrix Testerを使用し、以下の方法のいずれかで基板の導通を確認します。
- スイッチをはめ、押してみて反応を見る。
...
### 組み立て
まずバックパネルの、文字の書いてある面からネジ(長)を通します。このネジはスペーサー(短)で固定します。  
次にバックパネルのネジに合うように基板を重ね、ネジを露出させます。  
ネジにスペーサー(長)を取り付け、スイッチプレートを重ねたら、ネジ(短)で固定します。

## ゴム足の取付け
裏側パネルにゴム足を取り付けます。  
左側は文字が避けるようになっているので、位置を合わせて貼りつけてください。  
右側は文字の高さを元に左と合わせるか、定規などを使用して位置を合わせます。  
大小2個ずつ使用してチルトさせる場合、下側のゴム足は
