...
販路やイベントにより販売時の内容は異なります。足りないものをそれぞれ揃えてください。
 ...
 - ダイオード  
   73個 1N4148シリーズ 表面実装、リードどちらを使用しても構いません。
 - MX用 2Uスタビライザー  
   3個　PCBマウントタイプ
...
以下のバージョンでは問題点が発見されています。確認の上対応をお願いします。  
 - Ver.1.0
   - D73のシルクスクリーンが見づらい。  
D1のすぐ隣にあります。シルクのある面から見て左側がD73です。
   - D10とD15、L63の配線が交雑している。  
V1.0基板はすべてこの部分を修正してから販売しています。ダイオード、ジャンパー線、LEDは取り外さずに使用してください。
...
<details><sumamry>LEDが正しくはんだづけできているか不安…</sumamry>コンスルーを使用し、基板とMCUを接続してみてください。問題なくはんだづけできれていれば、通電すると赤色に光るはずです。この状態でVialを起動し、Backlightingのタブから色やアニメーションを切り替えましょう。問題なく接続できているなら、アニメーションがなめらかに切り替わりますが、接続が不安定だと、その部分から先のLEDは不安定になります。</details>
...
この時点で[導通確認](#matrixcheck)を行うことも可能です。
...
### 組み立て(付属のスペーサーが丸形の場合)
バックパネルの、文字の書いてある面からネジ(長)を通したら、マスキングテープでネジの頭を留めていきます。  
すべてのネジを留めた段階で基板を裏返し、平たい面に置いたら、上向きに露出しているネジにスペーサー(短)を通します。  
基板をネジに合うように重ね、スペーサー(長)を取り付けてしっかり固定します。  
この状態でスイッチプレートを重ね、ネジ(短)で固定します。
...
左上のエリア、Esc以外の4キーは初期状態ではLEDの制御に使用しています。ShiftやFnを押しながら操作することで機能を切り替えられます。  
