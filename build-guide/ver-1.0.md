...
1N4148Wシリーズの使用を想定しています。向きは左向きに揃えてあります。  
...
<details><summary>実際認識に使っているピンはどれ？</summary>基板に実装したときに、横にシルクスクリーンが付いているピン、つまりGP1,2,3,4,5,6,7,8,14,15,26,27,28です。これ以外のピンに機能は振っておらず、5Vや3V3、GNDも未使用かつ未接続です。</details>
...
スイッチプレートと基板を重ね、基板の穴にスペーサーを通します。この状態で、スイッチプレートの表側からネジを通して固定します。  

### スイッチの取付け
![スイッチをはめます](img/switch.jpg)  
スイッチプレートを正面から見て、足が上側になるようにスイッチを取り付けます。  
RP2040-Zero付近のSW31とSW35だけは足が下側になります。    
...
# マッピングガイド
キーマップの書き換えにはVialまたはReMap for for QMK Firmware 0.18.17 or lowerを使用します。どちらも日本語配列にも対応しているため使いやすいと思います。  
