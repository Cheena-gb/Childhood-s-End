...
### 内容物の確認
 - 基板  
   1枚入っています。ケース・ケースレスどちらも同じ基板です。  
 - スイッチプレート  
   1枚入っています。ケースバージョンでは上下左右にマージン(ねじ止めのためのスペース)があり、ケースレスバージョンでは上下にのみマージンがあります。  
 ...
 - 外周パネル(ケースバージョンのみ)  
   1枚入っています。  
 - ネジ  
   ケースバージョンでは短いものが8本と長いものが8本、ケースレスバージョンでは短いものが16本必要になります。
   余裕を持って少し多めに入っているはずです。  
 - ゴム足  
   4枚入っています。  
### 必須パーツの用意
 - Pro Micro  
...
バラ買いしたり、移植する必要があるキーは以下の通りです。
 - 右ShiftおよびFn  
   1U @R2が必要です。テンキーや矢印キー上を使用してください。  
 - Space  
...
全て本リポジトリ内で入手できます。
### childhood_s_end_via-****.hex
キーボードを制御するためのファームウェアです。キーマップが崩れてしまうことがあるため、初期キーマップは完全に空に設定しています。
### childhood_s_end_via-****.json
VIA用の初期キーマップ定義用ファイルです。最初に1回使用したのちは基本的に使用しません。
### info-****.json
...
向きは基本的に全て揃えてあります。D1、D2、D30のみ下向きです。  
...
## 筐体への取付け(ケースバージョン)
![Case Work](img/case.jpg)  
筐体にかぶせるようにスイッチプレートと基板を収めます。外周パネルを使用する場合はキーを囲うように載せます。
全てのネジ穴と筐体に埋め込まれたナットの位置を合わせます。  
付属のネジで固定して、ゴム足を取り付けたら完成です。

## バックパネルの取付け(ケースレスバージョン)
![Panel Work](img/caseless.jpg)  
